/** Supported UI languages. */
export type Language = "vi" | "en";

/** Text with both Vietnamese and English strings. */
export type LocalizedText = Record<Language, string>;

/** Language picker options for settings UI. */
export const languageOptions: Array<{
  label: string;
  value: Language;
}> = [
  {
    label: "Tiếng Việt",
    value: "vi",
  },
  {
    label: "English",
    value: "en",
  },
];

/**
 * Full UI copy bag keyed by language.
 *
 * Large nested structure used by the web app; prefer {@link AppCopy} for typing.
 */
export const translations = {
  vi: {
    activity: {
      closeModal: "Đóng chọn hoạt động",
      customTooltip: "Muốn tự nhập? Chọn mục này cho hoạt động chưa có sẵn.",
      labels: {
        other: "Hoạt động khác",
        study: "Học tập",
        workout: "Tập luyện",
      },
      modalTitle: "Chọn hoạt động",
      selectActivity: "Chọn hoạt động",
      selectedActivity: (activity: string) => `Hoạt động đã chọn: ${activity}`,
    },
    activityTimeline: {
      duration: (minutes: number) => `${minutes} phút`,
      empty: "Chưa có hoạt động nào trong chế độ xem này.",
      imageCount: (count: number) => `${count} ảnh`,
      loadError: "Chưa thể tải hoạt động.",
      loading: "Đang tải...",
      loadMore: "Xem thêm",
      openWorkout: (type: string, timestamp: string) =>
        `Mở hoạt động ${type} đăng lúc ${timestamp}`,
      postedAt: (timestamp: string) => `Đăng ${timestamp}`,
      private: "Riêng tư",
      title: "Contribution activity",
    },
    common: {
      cancel: "Hủy",
      close: "Đóng",
      edit: "Chỉnh sửa",
      home: "Trang chủ",
      language: "Ngôn ngữ",
      minutes: "phút",
      profile: "Hồ sơ",
      retry: "Thử lại",
      saveChanges: "Lưu thay đổi",
      saving: "Đang lưu...",
      settings: "Cài đặt",
      signIn: "Đăng nhập",
      signOut: "Đăng xuất",
    },
    settings: {
      account: "Tài khoản",
      appearance: "Giao diện",
      profile: "Hồ sơ công khai",
    },
    dashboard: {
      addWorkout: "Thêm buổi tập",
      ageMap: "Tuổi trên bản đồ",
      avatarUploadFailed: "Chưa thể tải ảnh đại diện lên.",
      avatarCropCanvas: "Di chuyển ảnh trong vùng cắt ảnh đại diện",
      avatarCropFailed: "Chưa thể cắt ảnh đã chọn.",
      avatarCropInstruction: "Kéo ảnh hoặc dùng phím mũi tên để định vị",
      avatarCropPublic: "Ảnh đại diện của bạn được hiển thị công khai.",
      avatarCropSave: "Lưu ảnh",
      avatarCropSaving: "Đang lưu...",
      avatarCropTitle: "Điều chỉnh ảnh đại diện",
      avatarCropZoom: "Thu phóng",
      avatarCropZoomIn: "Phóng to",
      avatarCropZoomOut: "Thu nhỏ",
      avatarInvalidType: "Vui lòng chọn ảnh JPG, PNG hoặc WebP.",
      avatarTooLarge: "Ảnh đại diện phải có dung lượng từ 5MB trở xuống.",
      avatarUploadPending: "Ảnh mới đang được lưu. Bạn vẫn có thể thấy preview trong lúc chờ.",
      avatarUpdated: "Đã cập nhật ảnh đại diện.",
      birthYear: "Năm sinh",
      closeWorkoutForm: "Đóng biểu mẫu tập luyện",
      facebook: "Facebook",
      appName: "Hope",
      instagram: "Instagram",
      linkedin: "LinkedIn",
      location: "Vị trí",
      editProfile: "Chỉnh sửa hồ sơ",
      loadingJokes: [
        "Đang đếm rep cho pixel...",
        "Nhờ server giữ streak giúp...",
        "Làm ấm cơ ngón tay cái...",
        "Ép ảnh chịu tập chung...",
        "Đang tìm động lực trong loading bar...",
        "Khuyên mồ hôi đừng bay mất...",
        "Thêm một chút hy vọng vào file...",
        "Đang vẽ heatmap bằng ý chí...",
        "Nhắc hôm nay bạn đã cố gắng...",
        "Gói buổi tập thành bưu kiện dễ thương...",
      ],
      managePlan: "Quản lý gói",
      open3dMap: "Mở bản đồ 3D",
      pronouns: "Đại từ",
      trackingFrom: "Theo dõi từ",
      upgradeToPro: "Nâng cấp Pro",
      uploadAvatar: "Tải ảnh đại diện",
      uploadingAvatar: "Đang tải ảnh...",
      website: "Website",
      years: "tuổi",
    },
    profileSettings: {
      backToProfile: "Quay lại hồ sơ",
      backToSettings: "Quay lại cài đặt",
      basicDescription: "Tên hiển thị và năm bắt đầu dòng thời gian của bạn.",
      basicTitle: "Thông tin cơ bản",
      bioEnglish: "Giới thiệu bằng tiếng Anh",
      bioVietnamese: "Giới thiệu bằng tiếng Việt",
      birthYear: "Năm sinh",
      description: "Cập nhật thông tin công khai hiển thị trên trang hồ sơ.",
      displayName: "Tên hiển thị",
      facebook: "Facebook",
      instagram: "Instagram",
      linkedin: "LinkedIn",
      linksDescription: "Để trống bất kỳ liên kết nào bạn muốn xóa khỏi hồ sơ.",
      linksTitle: "Liên kết",
      manageBilling: "Quản lý thanh toán",
      planDescription:
        "Standard ghi buổi tập theo ngày. Pro cho phép chỉnh sửa buổi tập cũ và mở thêm tính năng sau này.",
      planPro: "Pro",
      planStandard: "Standard",
      planTitle: "Gói hiện tại",
      profileDescription: "Viết riêng nội dung tiếng Việt và tiếng Anh cho khách xem hồ sơ.",
      profileTitle: "Giới thiệu công khai",
      pronounsEnglish: "Đại từ bằng tiếng Anh",
      pronounsVietnamese: "Đại từ bằng tiếng Việt",
      saveFailed: "Chưa thể cập nhật hồ sơ.",
      saveSuccess: "Đã lưu thay đổi hồ sơ.",
      title: "Chỉnh sửa hồ sơ",
      upgradeToPro: "Nâng cấp Pro",
      usernameLocked: "Username và đường dẫn hồ sơ được quản lý bởi Clerk.",
      validationFailed: "Vui lòng kiểm tra lại các trường được đánh dấu.",
      website: "Website",
    },
    errors: {
      imageLimit: (limit: number) => `Chọn tối đa ${limit} ảnh.`,
      imagePreviewFailed: "Chưa thể tạo preview cho ảnh HEIC này. Vui lòng thử ảnh khác.",
      invalidJson: (fallback: string) => `${fallback} Máy chủ trả về dữ liệu JSON không hợp lệ.`,
      nonJson: (fallback: string) => `${fallback} Máy chủ không trả về dữ liệu JSON.`,
      saveWorkout: "Chưa thể lưu buổi tập.",
      typeRequired: "Vui lòng chọn hoạt động.",
      updateWorkout: "Chưa thể cập nhật buổi tập.",
      workoutLoad: "Chưa thể tải dữ liệu tập luyện.",
    },
    form: {
      addImages: "Thêm ảnh",
      cameraCapture: "Chụp ảnh",
      cameraCaptureFailed: "Chưa thể chụp ảnh từ camera. Vui lòng thử lại.",
      cameraCapturing: "Đang chụp...",
      cameraDialogDescription: "Căn chỉnh khung hình, sau đó chụp để thêm ảnh vào buổi tập.",
      cameraDialogTitle: "Chụp ảnh bằng webcam",
      cameraInUse: "Camera đang được ứng dụng khác sử dụng. Hãy đóng ứng dụng đó rồi thử lại.",
      cameraLivePreview: "Hình ảnh trực tiếp từ webcam",
      cameraNotFound: "Không tìm thấy camera khả dụng trên thiết bị này.",
      cameraPermissionDenied:
        "Quyền truy cập camera bị từ chối. Hãy cấp quyền trong trình duyệt rồi thử lại.",
      cameraStarting: "Đang mở camera...",
      cameraUnavailable: "Trình duyệt này không hỗ trợ camera hoặc trang chưa được mở qua HTTPS.",
      date: "Ngày",
      editWorkout: "Sửa buổi tập",
      dropImages: "Kéo thả ảnh vào đây",
      imageFormats: "JPG, PNG, WebP, HEIC · tối đa 3 ảnh, 10 MB/ảnh",
      imageLimitReached: (limit: number) => `Đã đủ ${limit} ảnh`,
      imageSlotAvailable: (count: number) => `Còn ${count} vị trí ảnh có thể thêm.`,
      images: "Ảnh",
      existingImages: (count: number) => `${count} ảnh hiện có`,
      logWorkout: "Ghi lại hoạt động của bạn",
      newImages: "Ảnh mới",
      note: "Caption",
      captionPills: "Gợi ý nhanh",
      captionPillOptions: [
        "25°C",
        "Good morning",
        "Good night",
        "Đi chill",
        "Vừa xong",
        "Một ngày đẹp",
      ],
      notePlaceholder: "Caption ngắn cho tấm hình",
      publicWorkout: "Buổi tập công khai",
      publicWorkoutHelp: "Tắt để chỉ mình bạn xem được buổi tập này.",
      preparingImages: "Đang tạo preview ảnh",
      removeImage: (index: number) => `Xóa ảnh ${index}`,
      selectedImageCount: (count: number, limit: number) => `Đã chọn ${count} trên ${limit} ảnh`,
      selectedEditPreviewAlt: (index: number) => `Ảnh chỉnh sửa đã chọn ${index}`,
      selectedPreviewAlt: (index: number) => `Ảnh buổi tập đã chọn ${index}`,
      submit: "Lưu lại",
      success: "Đã lưu buổi tập và cập nhật heatmap.",
      takePhoto: "Chụp ảnh",
      todayEntry: "Hôm nay",
      type: "Loại",
      workoutType: "Hoạt động",
      workoutTypePlaceholder: "Đi bộ, gym, chạy bộ",
      workoutUpdated: "Đã cập nhật buổi tập.",
      webcam: "Webcam",
    },
    header: {
      dark: "Tối",
      english: "English",
      light: "Sáng",
      savingTheme: "Đang lưu giao diện...",
      searchError: "Chưa thể tìm người dùng.",
      searchLoading: "Đang tìm...",
      searchNoResults: "Không tìm thấy người dùng.",
      searchPlaceholder: "Tìm người dùng",
      theme: "Giao diện",
      themeUpdateFailed: "Chưa thể lưu giao diện.",
      vietnamese: "Tiếng Việt",
    },
    heatmap: {
      lifetime: "Trọn đời",
      less: "Ít hơn",
      intensityEmpty: "Không buổi nào",
      intensityLevel: (count: number) => (count >= 4 ? "4+ buổi tập" : `${count} buổi tập`),
      intensityLegend: "Cường độ màu theo số buổi trong ngày",
      intensityScaleHint: "Màu đậm hơn = nhiều buổi hơn trong ngày đó (1 → 4+ buổi).",
      months: [
        "Th1",
        "Th2",
        "Th3",
        "Th4",
        "Th5",
        "Th6",
        "Th7",
        "Th8",
        "Th9",
        "Th10",
        "Th11",
        "Th12",
      ],
      more: "Nhiều hơn",
      noData: "Chưa có dữ liệu",
      noTrackingYet: "Chưa bắt đầu theo dõi",
      noWorkout: "Chưa tập",
      scrollHint: "Vuốt ngang để xem cả năm",
      weekdays: ["CN", "T2", "T3", "T4", "T5", "T6", "T7"],
      view: "Chế độ xem",
      workout: "Có tập",
      workoutCount: (count: number) => `${count} buổi tập`,
      yearTitle: (year: number) => `Năm ${year}`,
    },
    navigation: {
      overview: "Tổng quan",
      workouts: "Buổi tập",
    },
    workoutsList: {
      empty: "Chưa có buổi tập nào.",
      title: "Buổi tập",
    },
    modal: {
      addMoreImages: (count: number) => `Thêm tối đa ${count} ảnh nữa.`,
      backToEditWorkout: "Quay lại chỉnh sửa",
      backToWorkoutDetail: "Quay lại chi tiết buổi tập",
      closeWorkoutDetail: "Đóng chi tiết buổi tập",
      dayDetail: "Chi tiết ngày",
      editWorkoutImageGallery: "Ảnh buổi tập",
      expandWorkoutImage: "Phóng lớn ảnh buổi tập",
      nextWorkoutImage: "Xem ảnh tiếp theo",
      previousWorkoutImage: "Xem ảnh trước",
      loggedWorkoutCount: (count: number) => `${count} buổi tập đã ghi`,
      minimizeWorkoutImage: "Thu nhỏ ảnh",
      noWorkoutLogged: "Chưa ghi buổi tập.",
      showWorkoutImage: (index: number) => `Xem ảnh buổi tập ${index}`,
      zoomInWorkoutImage: "Phóng to ảnh",
      zoomOutWorkoutImage: "Thu nhỏ ảnh",
    },
    stats: {
      activeDays: "Ngày có tập",
      activeDaysDetail: (scope: string) => `đã ghi trong ${scope}`,
      activityMix: "Cơ cấu hoạt động",
      activityMixEmpty: "Chưa có buổi nào để phân loại.",
      activityCount: (_count: number) => "hoạt động",
      currentStreak: "Chuỗi hiện tại",
      day: "ngày",
      days: "ngày",
      last30Days: "30 ngày gần đây",
      last30DaysDetail: "ngày có vận động",
      dayActive: "Có hoạt động",
      dayIdle: "Chưa hoạt động",
      last7Days: "7 ngày gần nhất",
      last7DaysDetail: "Ngày có hoạt động",
      last30Pace: "Nhịp 30 ngày",
      last30PaceDetail: (activeDays: number) => `${activeDays} trong 30 ngày gần nhất có hoạt động`,
      last30Start: "30 ngày trước",
      last30EndToday: "Hôm nay",
      last30TooltipIdle: "Không hoạt động",
      last30TooltipCount: (count: number) => `${count} hoạt động`,
      lifetimeScope: "toàn bộ thời gian",
      personalBest: (days: number) => `Kỷ lục cá nhân: ${days} ngày`,
      personalBestEmpty: "Chưa có kỷ lục",
      streakMatchingBest: "Đang ngang kỷ lục cá nhân",
      streakDaysToBeat: (days: number) => `Còn ${days} ngày để vượt kỷ lục`,
      streakMissedToday: "Bạn đã bỏ lỡ hôm nay",
      streakLastActiveDaysAgo: (days: number) => `Hoạt động gần nhất ${days} ngày trước`,
      streakStart: "Bắt đầu chuỗi của bạn",
      streakStartAction: "Ghi một hoạt động hôm nay để bắt đầu chuỗi mới.",
      totalSessions: "Tổng buổi",
      totalSessionsDetail: (count: number) => `${count} buổi đã ghi`,
    },
  },
  en: {
    activity: {
      closeModal: "Close activity picker",
      customTooltip: "Want to enter your own? Choose this for activities not listed.",
      labels: {
        other: "Other activity",
        study: "Study",
        workout: "Workout",
      },
      modalTitle: "Choose an activity",
      selectActivity: "Choose an activity",
      selectedActivity: (activity: string) => `Selected activity: ${activity}`,
    },
    activityTimeline: {
      duration: (minutes: number) => `${minutes} min`,
      empty: "No activity in this view yet.",
      imageCount: (count: number) => `${count} image${count === 1 ? "" : "s"}`,
      loadError: "Unable to load activity.",
      loading: "Loading...",
      loadMore: "Show more",
      openWorkout: (type: string, timestamp: string) =>
        `Open ${type} activity posted at ${timestamp}`,
      postedAt: (timestamp: string) => `Posted ${timestamp}`,
      private: "Private",
      title: "Contribution activity",
    },
    common: {
      cancel: "Cancel",
      close: "Close",
      edit: "Edit",
      home: "Home",
      language: "Language",
      minutes: "minutes",
      profile: "Profile",
      retry: "Retry",
      saveChanges: "Save changes",
      saving: "Saving...",
      settings: "Settings",
      signIn: "Sign in",
      signOut: "Sign out",
    },
    settings: {
      account: "Account",
      appearance: "Appearance",
      profile: "Public profile",
    },
    dashboard: {
      addWorkout: "Add a workout",
      ageMap: "Age map",
      avatarUploadFailed: "Unable to upload avatar.",
      avatarCropCanvas: "Reposition the image inside the profile crop area",
      avatarCropFailed: "Unable to crop the selected image.",
      avatarCropInstruction: "Drag the image or use arrow keys to reposition",
      avatarCropPublic: "Your profile picture is public.",
      avatarCropSave: "Save photo",
      avatarCropSaving: "Saving...",
      avatarCropTitle: "Adjust profile picture",
      avatarCropZoom: "Zoom",
      avatarCropZoomIn: "Zoom in",
      avatarCropZoomOut: "Zoom out",
      avatarInvalidType: "Choose a JPG, PNG, or WebP image.",
      avatarTooLarge: "Profile pictures must be 5MB or smaller.",
      avatarUploadPending:
        "Your new avatar is being saved. The preview stays visible while you wait.",
      avatarUpdated: "Avatar updated.",
      birthYear: "Birth year",
      closeWorkoutForm: "Close workout form",
      facebook: "Facebook",
      appName: "Hope",
      instagram: "Instagram",
      linkedin: "LinkedIn",
      location: "Location",
      editProfile: "Edit profile",
      loadingJokes: [
        "Counting reps for the pixels...",
        "Asking the server to hold your streak...",
        "Warming up your thumbs...",
        "Convincing the photo it worked out too...",
        "Looking for motivation in the loading bar...",
        "Telling sweat it cannot evaporate yet...",
        "Packing a little hope into the file...",
        "Drawing the heatmap with sheer will...",
        "Reminding today that you showed up...",
        "Wrapping your session like a cute parcel...",
      ],
      managePlan: "Manage plan",
      open3dMap: "Open 3D map",
      pronouns: "Pronouns",
      trackingFrom: "Tracking from",
      upgradeToPro: "Upgrade to Pro",
      uploadAvatar: "Upload avatar",
      uploadingAvatar: "Uploading avatar...",
      website: "Website",
      years: "years",
    },
    profileSettings: {
      backToProfile: "Back to profile",
      backToSettings: "Back to settings",
      basicDescription: "Your display name and the year your timeline begins.",
      basicTitle: "Basic information",
      bioEnglish: "English bio",
      bioVietnamese: "Vietnamese bio",
      birthYear: "Birth year",
      description: "Update the public information shown on your profile page.",
      displayName: "Display name",
      facebook: "Facebook",
      instagram: "Instagram",
      linkedin: "LinkedIn",
      linksDescription: "Leave any link empty to remove it from your profile.",
      linksTitle: "Links",
      manageBilling: "Manage billing",
      planDescription:
        "Standard covers daily logging. Pro unlocks editing past workouts and future Pro features.",
      planPro: "Pro",
      planStandard: "Standard",
      planTitle: "Current plan",
      profileDescription: "Write separate Vietnamese and English copy for profile visitors.",
      profileTitle: "Public introduction",
      pronounsEnglish: "English pronouns",
      pronounsVietnamese: "Vietnamese pronouns",
      saveFailed: "Unable to update your profile.",
      saveSuccess: "Profile changes saved.",
      title: "Edit profile",
      upgradeToPro: "Upgrade to Pro",
      usernameLocked: "Your username and profile URL are managed by Clerk.",
      validationFailed: "Review the highlighted fields and try again.",
      website: "Website",
    },
    errors: {
      imageLimit: (limit: number) => `Choose up to ${limit} images.`,
      imagePreviewFailed: "Unable to preview this HEIC image. Please try another image.",
      invalidJson: (fallback: string) => `${fallback} The server returned invalid JSON.`,
      nonJson: (fallback: string) => `${fallback} The server returned a non-JSON response.`,
      saveWorkout: "Unable to save workout.",
      typeRequired: "Choose an activity.",
      updateWorkout: "Unable to update workout.",
      workoutLoad: "Unable to load workouts.",
    },
    form: {
      addImages: "Add images",
      cameraCapture: "Take photo",
      cameraCaptureFailed: "Unable to capture a photo from the camera. Please try again.",
      cameraCapturing: "Capturing...",
      cameraDialogDescription: "Frame your shot, then capture it to add the photo to your workout.",
      cameraDialogTitle: "Take a webcam photo",
      cameraInUse: "The camera is in use by another app. Close that app and try again.",
      cameraLivePreview: "Live webcam preview",
      cameraNotFound: "No available camera was found on this device.",
      cameraPermissionDenied: "Camera access was denied. Allow it in your browser and try again.",
      cameraStarting: "Starting camera...",
      cameraUnavailable:
        "This browser does not support camera access, or the page is not using HTTPS.",
      date: "Date",
      dropImages: "Drop images here",
      editWorkout: "Edit workout",
      imageFormats: "JPG, PNG, WebP, HEIC · up to 3 images, 10 MB each",
      imageLimitReached: (limit: number) => `${limit} images added`,
      imageSlotAvailable: (count: number) =>
        `${count} image slot${count === 1 ? "" : "s"} available.`,
      images: "Images",
      existingImages: (count: number) => `${count} existing image${count === 1 ? "" : "s"}`,
      logWorkout: "Record your activity",
      newImages: "New images",
      note: "Caption",
      captionPills: "Quick captions",
      captionPillOptions: [
        "25°C",
        "Good morning",
        "Good night",
        "Post-workout",
        "Just now",
        "Golden hour",
      ],
      notePlaceholder: "A short caption for the photo",
      publicWorkout: "Public workout",
      publicWorkoutHelp: "Turn off to make this workout visible only to you.",
      preparingImages: "Preparing image previews",
      removeImage: (index: number) => `Remove image ${index}`,
      selectedImageCount: (count: number, limit: number) => `${count} of ${limit} images selected`,
      selectedEditPreviewAlt: (index: number) => `Selected edit preview ${index}`,
      selectedPreviewAlt: (index: number) => `Selected workout preview ${index}`,
      submit: "Submit",
      success: "Workout saved and heatmap refreshed.",
      takePhoto: "Take photo",
      todayEntry: "Today's entry",
      type: "Type",
      workoutType: "Activity",
      workoutTypePlaceholder: "Walking, gym, running",
      workoutUpdated: "Workout updated.",
      webcam: "Webcam",
    },
    header: {
      dark: "Dark",
      english: "English",
      light: "Light",
      savingTheme: "Saving theme...",
      searchError: "Unable to search users.",
      searchLoading: "Searching...",
      searchNoResults: "No users found.",
      searchPlaceholder: "Search users",
      theme: "Theme",
      themeUpdateFailed: "Unable to save theme.",
      vietnamese: "Tiếng Việt",
    },
    heatmap: {
      lifetime: "Lifetime",
      less: "Less",
      intensityEmpty: "No sessions",
      intensityLevel: (count: number) =>
        count >= 4 ? "4+ sessions" : `${count} session${count === 1 ? "" : "s"}`,
      intensityLegend: "Color intensity by sessions that day",
      intensityScaleHint: "Darker to brighter means more sessions logged that day (1 → 4+).",
      months: ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"],
      more: "More",
      noData: "No data",
      noTrackingYet: "No tracking yet",
      noWorkout: "No workout",
      scrollHint: "Swipe sideways to see the full year",
      weekdays: ["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"],
      view: "View",
      workout: "Workout",
      workoutCount: (count: number) => `${count} workout${count === 1 ? "" : "s"}`,
      yearTitle: (year: number) => `Your ${year}`,
    },
    navigation: {
      overview: "Overview",
      workouts: "Workouts",
    },
    workoutsList: {
      empty: "No workouts yet.",
      title: "Workouts",
    },
    modal: {
      addMoreImages: (count: number) => `Add up to ${count} more image${count === 1 ? "" : "s"}.`,
      backToEditWorkout: "Back to edit",
      backToWorkoutDetail: "Back to workout detail",
      closeWorkoutDetail: "Close workout detail",
      dayDetail: "Day detail",
      editWorkoutImageGallery: "Workout images",
      expandWorkoutImage: "Expand workout image",
      nextWorkoutImage: "Show next image",
      previousWorkoutImage: "Show previous image",
      loggedWorkoutCount: (count: number) => `${count} workout${count === 1 ? "" : "s"} logged`,
      minimizeWorkoutImage: "Minimize image",
      noWorkoutLogged: "No workout logged.",
      showWorkoutImage: (index: number) => `Show workout image ${index}`,
      zoomInWorkoutImage: "Zoom image in",
      zoomOutWorkoutImage: "Zoom image out",
    },
    stats: {
      activeDays: "Active days",
      activeDaysDetail: (scope: string) => `logged in ${scope}`,
      activityMix: "Activity mix",
      activityMixEmpty: "No sessions to chart yet.",
      activityCount: (count: number) => (count === 1 ? "activity" : "activities"),
      currentStreak: "Current streak",
      day: "day",
      days: "days",
      last30Days: "Last 30 days",
      last30DaysDetail: "days with movement",
      dayActive: "Active",
      dayIdle: "No activity",
      last7Days: "Last 7 days",
      last7DaysDetail: "Active days",
      last30Pace: "30-day pace",
      last30PaceDetail: (activeDays: number) => `${activeDays} of the last 30 days active`,
      last30Start: "30 days ago",
      last30EndToday: "Today",
      last30TooltipIdle: "No activity",
      last30TooltipCount: (count: number) => `${count} activit${count === 1 ? "y" : "ies"}`,
      lifetimeScope: "lifetime view",
      personalBest: (days: number) => `Personal best: ${days} days`,
      personalBestEmpty: "No personal best yet",
      streakMatchingBest: "Matching your personal best",
      streakDaysToBeat: (days: number) => `${days} day${days === 1 ? "" : "s"} to beat your best`,
      streakMissedToday: "You missed today",
      streakLastActiveDaysAgo: (days: number) =>
        `Last activity ${days} day${days === 1 ? "" : "s"} ago`,
      streakStart: "Start your streak",
      streakStartAction: "Log an activity today to start a new streak.",
      totalSessions: "Total sessions",
      totalSessionsDetail: (count: number) => `${count} session${count === 1 ? "" : "s"} logged`,
    },
  },
} as const;

/** Copy bag for one language from {@link translations}. */
export type AppCopy = (typeof translations)[Language];
