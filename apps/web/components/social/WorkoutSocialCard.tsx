...
export function WorkoutSocialCard({
  ...
  async function toggleLike() {
    ...
    try {
      ...
      const res = nextLiked
        ? await client.workouts[":workoutId"].like.$post({ param: { workoutId: item.workout.id } })
        : await client.workouts[":workoutId"].like.$delete({
            param: { workoutId: item.workout.id },
          });
      const payload = await res.json();
      if (!res.ok) throw new Error(("error" in payload && payload.error) || copy.interactionFailed);
      setLiked("viewerHasLiked" in payload ? payload.viewerHasLiked : nextLiked);
      setLikeCount(
        "likeCount" in payload ? payload.likeCount : previousCount + (nextLiked ? 1 : -1),
      );
    } catch (caught) {
  ...
  async function submitComment(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const payload = await res.json();
      if (!res.ok) throw new Error(("error" in payload && payload.error) || copy.interactionFailed);
      const comment = "comment" in payload ? payload.comment : null;
      if (comment) {
  ...
  async function updateComment(commentId: string, body: string) {
    ...
    try {
      const token = await getToken();
      const client = getClientApiClient(token);
      const res = await client.comments[":commentId"].$patch({
        param: { commentId },
        json: { body },
      });
      const payload = await res.json();
      if (!res.ok) throw new Error(("error" in payload && payload.error) || copy.interactionFailed);
      const updated = "comment" in payload ? payload.comment : null;
      if (updated) {
        setComments((current) =>
  ...
  async function deleteComment(commentId: string) {
    ...
    try {
      const token = await getToken();
      const client = getClientApiClient(token);
      const res = await client.comments[":commentId"].$delete({ param: { commentId } });
      if (!res.ok) {
        const data = await res.json();
        throw new Error(("error" in data && data.error) || copy.interactionFailed);
      }
    } catch (caught) {
      setComments(previous);
      setCommentCount((value) => value + 1);
  ...
  async function loadEarlierComments() {
    ...
    try {
      ...
      const res = await client.workouts[":workoutId"].comments.$get({
        param: { workoutId: item.workout.id },
        query: { cursor: commentsCursor },
      });
      const payload = await res.json();
      if (!res.ok) throw new Error(("error" in payload && payload.error) || copy.interactionFailed);
      const items = "items" in payload ? payload.items : [];
      setComments((current) => [...items, ...current]);
      setCommentsCursor("nextCursor" in payload ? payload.nextCursor : null);
    } catch (caught) {
      setError(getApiErrorMessage(caught, copy.interactionFailed));
    } finally {
