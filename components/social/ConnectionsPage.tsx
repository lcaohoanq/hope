...
export async function ConnectionsPage({
  ...
  const target = await getProfileByPath(userSlug);
  if (!target) notFound();
  const { userId } = await auth();
  const viewer = userId ? await getProfileByClerkId(userId) : undefined;
  const access = await resolveProfileAccess(target, viewer);
  const copy = getSocialCopy(viewer?.preferredLanguage ?? target.preferredLanguage);
  return (
    <main className="min-h-dvh bg-app text-text">
      <div className="mx-auto max-w-2xl px-4 py-8 sm:px-6">
        <Link className="text-sm font-semibold text-accent" href={`/${target.username}`}>
          ← @{target.username}
        </Link>
        ...
        <ConnectionsPageClient
          canView={access.canViewConnections}
          language={viewer?.preferredLanguage ?? target.preferredLanguage}
          profileId={target.id}
          type={type}
        />
      </div>
