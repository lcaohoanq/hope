...
export function WorkoutTooltip({
  ...
  return (
    <div className="w-64 rounded-lg border border-border bg-panel p-3 text-left text-xs text-muted shadow-[0_18px_48px_rgba(17,17,17,0.10)]">
      <p className="font-medium text-text">{formatDisplayDate(date, language)}</p>
      {!isTrackable ? (
        ...
        <div className="mt-2 space-y-2">
          {workouts.length > 1 ? (
            <p className="font-medium text-accent">{copy.heatmap.workoutCount(workouts.length)}</p>
          ) : null}
                ...
                <div className="grid grid-cols-3 gap-1.5 pt-1">
                  {workout.images.map((image) => (
                    <div
                      ...
                      key={image.src}
                    >
                      <WorkoutImageThumbnail image={image} workoutDate={workout.date} />
                    </div>
                  ))}
                </div>
              ) : null}
