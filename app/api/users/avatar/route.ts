...
export async function POST(request: Request) {
  ...
  if (owner.status === "onboarding") return NextResponse.json({ success: false, error: "Complete onboarding before uploading an avatar." }, { status: 403 });
  ...
  if (!(avatar instanceof File)) return NextResponse.json({ success: false, error: "An avatar image is required." }, { status: 400 });
  if (!AVATAR_MIME_TYPES.has(avatar.type)) return NextResponse.json({ success: false, error: "Please upload a JPG, PNG, or WebP image." }, { status: 400 });
  if (avatar.size > MAX_AVATAR_BYTES) return NextResponse.json({ success: false, error: "Avatar image must be 5MB or smaller." }, { status: 400 });
  ...
  try {
    ...
    return NextResponse.json({ success: false, error: "Unable to process avatar image." }, { status: 500 });
  }

  try {
    const updated = await updateProfileAvatar({ profileId: owner.profile.id, avatarUrl: asset.secureUrl, avatarPublicId: asset.publicId });
  ...
  if (owner.profile.avatarPublicId && owner.profile.avatarPublicId !== asset.publicId) {
    void deleteImage(owner.profile.avatarPublicId).catch((error) => console.error("Unable to delete replaced avatar.", error));
  }
