...
export async function PATCH(request: Request) {
  ...
  if (owner.status === "onboarding") return NextResponse.json({ success: false, error: "Complete onboarding before updating settings." }, { status: 403 });
  ...
  if (!isAppTheme(body.theme)) return NextResponse.json({ success: false, error: "Theme must be light or dark." }, { status: 400 });
  ...
  try {
    ...
    return NextResponse.json({ success: false, error: "Unable to update user settings." }, { status: 500 });
  }
}
