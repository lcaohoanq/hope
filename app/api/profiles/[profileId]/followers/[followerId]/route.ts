...
export async function DELETE(_: Request, { params }: RouteContext) {
  ...
  if (profileId !== owner.profile.id) return NextResponse.json({ success: false, error: "Follower was not found." }, { status: 404 });
  const removed = await removeFollower(profileId, followerId);
  return removed ? NextResponse.json({ success: true }) : NextResponse.json({ success: false, error: "Follower was not found." }, { status: 404 });
}
