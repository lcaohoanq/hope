...
export async function GET(request: Request, { params }: RouteContext) {
  ...
  if (!access.canViewConnections) return NextResponse.json({ success: false, error: "This connection list is private." }, { status: 403 });
  const url = new URL(request.url);
  const type = url.searchParams.get("type");
  if (type !== "followers" && type !== "following") return NextResponse.json({ success: false, error: "Connection type is invalid." }, { status: 400 });
  const result = await listConnections(profileId, type, viewer?.id, url.searchParams.get("cursor") ?? undefined);
