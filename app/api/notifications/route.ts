...
export async function GET(request: Request) {
  ...
  return NextResponse.json({ success: true, ...(await listNotifications(owner.profile.id, cursor)) });
}
...
export async function PATCH(request: Request) {
  ...
  const notificationId = body && typeof body === "object" && typeof (body as { notificationId?: unknown }).notificationId === "string"
